...
function AdminList() {
  ...
  const {
    ...
    loading,
    error,
  } = useFetchData("admin", [successDate], true);
  ...
  const updateDoctorStatus = async (id, status) => {
    try {
      ...
      toast.error(
        error.error || "An error occurred while updating doctor status"
      );
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await apiService.post("admin", {...formData,phone:formData.contactNumber}, true, false);
      ...
      if (response.success) {
        ...
        console.log(error)
      toast.error(
        error?.error?.[0].error || "Une erreur s'est produite lors de l'ajout du médecin"
      );
    } finally {
  ...
  // Calculate pagination - keeping original logic for backward compatibility
  ...
  return (
    <>
      <div className="flex flex-col items-center mt-8 intro-y sm:flex-row">
        <h2 className="mr-auto text-lg font-medium">Liste des admin</h2>
            ...
            <div>
              {centerResponse && centerResponse.data && (
                <Menu>
                  ...
                  <Menu.Items placement="top-start" className="h-64 overflow-y-auto">
                    {centerResponse.data.map((center) => (
                      <Menu.Item
                        key={center._id}
                        onClick={() =>
                          handleCenterSelect(center._id, center.name)
                        }
                      >
